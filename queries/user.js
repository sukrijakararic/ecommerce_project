...
const getUserById = async (id) => {
  ...
  try {
    const result = await db.query("SELECT * FROM users WHERE id = $1", [parsedId]);
...
const deleteUserById = async (request, response, next) => {
  ...
  try {
    const result = await db.query("DELETE FROM users WHERE id = $1", [parsedId]);
...
module.exports = {registerUser, getUserByEmail, showUsers, getUserById, changePAssword, deleteUserById, getUserByIdForRouter};
